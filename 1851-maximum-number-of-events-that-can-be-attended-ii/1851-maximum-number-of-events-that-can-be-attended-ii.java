class Solution {
    public int maxValue(int[][] events, int k) {
        ...
        Arrays.sort(events, Comparator.comparingInt((int[] event) -> event[0]).thenComparing((int[] event) -> event[1]));
    ...
    private int maxValue(int[][] events, int i, int k, Integer[][] mem) {
        ...
        return mem[i][k] = Math.max(events[i][2] + maxValue(events, j, k - 1, mem), maxValue(events, i + 1, k, mem));
    }
